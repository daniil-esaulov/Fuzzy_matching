...
sdf2$ed_city <- gsub("Ц", " ", sdf2$ed_city)
sdf2$ed_city <- gsub("Ч", " ", sdf2$ed_city)

#Change hyphen "-" and dash " - " to space " " in "firm" column
sdf2$ed_firm <- gsub("-", " ", sdf2$ed_firm)
sdf2$ed_firm <- gsub("Ц", " ", sdf2$ed_firm)
sdf2$ed_firm <- gsub("Ч", " ", sdf2$ed_firm)
...
#standarize  "є..."
sdf2$ed_firm <- gsub("є ", "є", sdf2$ed_firm)
sdf2$ed_firm_par_in <- gsub("є ", "є", sdf2$ed_firm_par_in)
sdf2$ed_firm_par_out <- gsub("є ", "є", sdf2$ed_firm_par_out)

sdf2$ed_city <- gsub("є ", "є", sdf2$ed_city)
...
for(i in region) {
  ...
  Ruslana <- Ruslana[(Ruslana$status == "ƒействующее")|(Ruslana$status_change_year >=2013), ]
  ...
  Ruslana$ed_firm_short <- gsub("-|Ц|Ч"," ", enc2utf8(Ruslana$ed_firm_short))
  Ruslana$ed_firm_long <- gsub("-|Ц|Ч"," ", enc2utf8(Ruslana$ed_firm_long))
  Ruslana$ed_city_rusl <- gsub("-|Ц|Ч"," ", enc2utf8(Ruslana$ed_city_rusl))
  ...
  #edit "є..."
  Ruslana$ed_firm_short <- gsub("є ", "є", Ruslana$ed_firm_short)
  Ruslana$ed_firm_long <- gsub("є ", "є", Ruslana$ed_firm_long)
  Ruslana$ed_city_rusl <- gsub("є ", "є", Ruslana$ed_city_rusl)
  ...
  sum$form <- gsub("муниципальное унитарное предпри€тие", "муп", sum$form)
  sum$form <- gsub("индивидуальный предприниматель", "ип", sum$form)
  sum$form <- gsub("товарищество собственников жиль€", "тсж", sum$form)
  sum$form <- gsub("потребительское общество", "по", sum$form)
  sum$form <- gsub("сельскохоз€йственный производственный кооператив", "спк|схпк", sum$form)
  sum$form <- gsub("потребительское общество", "по", sum$form)
  sum$form <- gsub("непубличное акционерное общество", "нао", sum$form)
  sum$form <- gsub("публичное акционерное общество", "пао", sum$form)
  sum$form <- gsub("муниципальное предпри€тие", "мп", sum$form)
  ...
  sum$form <- gsub("федеральное государственное унитарное предпри€тие", "фгуп", sum$form)
  sum$form <- gsub("индивидуальное частное предпри€тие", "ичп", sum$form)
  sum$form <- gsub("закрытое акционерное общество", "зао", sum$form)
  sum$form <- gsub("открытое акционерное общество", "оао", sum$form)
  sum$form <- gsub("автономна€ некоммерческа€ организаци€", "ано", sum$form)
  ...
  sum$form <- gsub("государственное предпри€тие", "гп", sum$form)
  sum$form <- gsub("государственное унитарное препдри€тие субъектов российской федерации", "гуп", sum$form)
  sum$form <- gsub("муниципальное автономное учреждение", "мау", sum$form)
  sum$form <- gsub("муниципальное казенное предпри€тие", "мкп", sum$form)
  sum$form <- gsub("муниципальное казенное учреждение", "мку", sum$form)
  sum$form <- gsub("профсоюзна€ организаци€", "по", sum$form)
  sum$form <- gsub("фермерское хоз€йство", "кфх|кх|фх", sum$form)
  sum$form <- gsub("кресть€нское фермерское хоз€йство юл", "кфх|кх|фх", sum$form)
  sum$form <- gsub("глава кресть€нского фермерского хоз€йства", "кфх|кх|фх", sum$form)
